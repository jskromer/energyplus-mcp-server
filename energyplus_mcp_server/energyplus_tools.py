...
from .utils.path_utils import resolve_path, PathResolver
...
class EnergyPlusManager:
    ...
    def run_simulation(self, idf_path: str, weather_file: str = None, 
            ...
            try:
              ...
              # Build EnergyPlus command directly (bypasses eppy's leaky subprocess)
                
                idf_name = Path(resolved_idf_path).stem
                ep_executable = self.config.energyplus.executable_path
                
                cmd = [ep_executable]
                
                if resolved_weather_path:
                    cmd.extend(['-w', resolved_weather_path])
                
                cmd.extend(['-d', output_directory])
                cmd.extend(['-p', idf_name])
                cmd.extend(['-s', 'C'])
                ...
                try:
                    # CRITICAL: capture_output=True prevents stdout pollution
                    process = subprocess.run(
                        cmd,
                        capture_output=True,
                        text=True,
                        cwd=output_directory,
                        timeout=self.config.server.simulation_timeout
                    )
                    ...
                    simulation_result = {
                        ...
                        "energyplus_result": "Simulation completed" if success else f"Exit code: {process.returncode}",
                        "timestamp": end_time.isoformat()
                    }
                    ...
                    simulation_result = {
                        ...
                        "weather_file": resolved_weather_path,
                        "output_directory": output_directory,
                        "error": f"Simulation timed out after {self.config.server.simulation_timeout} seconds",
                        "simulation_duration": str(duration),
                        "simulation_options": simulation_options,
                        "timestamp": end_time.isoformat()
                    }
                    ...
                    error_file = Path(output_directory) / f"{idf_name}.err"
                    error_details = ""
    ...
    # ------------------------ Unit Conversion ------------------------
    def convert_output_units(self, output_directory: str, idf_name: str = None,
        ...
        Args:
            ...
            target_energy_unit: Target unit for energy values. Options: "kWh", "MWh", "kBtu", "MBtu", "therm", "GJ", "MJ"
            target_power_unit: Target unit for power values. Options: "kW", "MW", "Btu/hr", "kBtu/hr", "ton"
            save_converted: If True, saves a new CSV with converted values
            file_type: "meter", "variable", or "auto" to detect automatically

        Returns:
            JSON string with conversion results and statistics
        """
        ...
        power_conversions = {
            ...
            "Btu/hr": 3.412142,
            "kBtu/hr": 3.412142e-3,
            "ton": 1 / 3516.85,  # refrigeration ton
            "hp": 1 / 745.7  # horsepower
        }
        ...
        try:
            ...
            if target_energy_unit not in energy_conversions:
                raise ValueError(f"Invalid target_energy_unit: {target_energy_unit}. Valid options: {list(energy_conversions.keys())}")
            if target_power_unit not in power_conversions:
                raise ValueError(f"Invalid target_power_unit: {target_power_unit}. Valid options: {list(power_conversions.keys())}")
                ...
                if csv_files:
                    for csv_file in csv_files:
                        if csv_file.name.endswith("Meter.csv"):
                            ...
                            break
                        elif not csv_file.name.endswith("Meter.csv") and not csv_file.name.endswith("_converted.csv"):
            ...
            for col in df.columns:
                ...
                # Detect energy columns (typically end with [J] or contain energy-related terms)
                if '[j]' in col_lower or col.endswith('(J)'):
                    # Convert from Joules to target energy unit
                    conversion_factor = energy_conversions[target_energy_unit]
                    df[col] = df[col] * conversion_factor

                    # Update column name
                    new_col = col.replace('[J]', f'[{target_energy_unit}]').replace('(J)', f'({target_energy_unit})')
                    ...
                    conversions_made.append({
                        ...
                        "new_column": new_col,
                        "from_unit": "J",
                        "to_unit": target_energy_unit,
                        "conversion_factor": conversion_factor
                    })

                # Detect power columns (typically end with [W])
                elif '[w]' in col_lower or col.endswith('(W)'):
                    # Convert from Watts to target power unit
                    conversion_factor = power_conversions[target_power_unit]
                    df[col] = df[col] * conversion_factor

                    # Update column name
                    new_col = col.replace('[W]', f'[{target_power_unit}]').replace('(W)', f'({target_power_unit})')
                    df.rename(columns={col: new_col}, inplace=True)
            ...
            for conv in conversions_made:
                new_col = conv["new_column"]
                if new_col in df.columns:
                    col_data = df[new_col].dropna()
                    if len(col_data) > 0:
                        summary_stats[new_col] = {
            ...
            result = {
                ...
                "columns_converted": len(conversions_made),
